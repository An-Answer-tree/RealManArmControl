...
class ToolCoordinateConfig:
    ...
    def rm_set_manual_tool_frame(self, name: str, pose: list, payload: float, center_of_mass: list) -> int:
        """
        手动设置工具坐标系

        Args:
            name (str): 工具坐标系名称，不超过十个字节
            pose (list): 工具坐标系相对初始末端的位姿(x, y, z, rx, ry, rz)
            payload (float): 负载质量(kg)
            center_of_mass (list): 负载工具的重心(x, y, z)
        ...
        """
        frame = rm_frame_t()
        frame.frame_name = name.encode()
        frame.pose.position = rm_position_t(*pose[0:3])
        frame.pose.euler = rm_euler_t(*pose[3:6])
        frame.payload = payload
        frame.x = center_of_mass[0]
        frame.y = center_of_mass[1]
        frame.z = center_of_mass[2]
    ...
    def rm_update_tool_frame(self, name, pose, payload, center_of_mass) -> int:
        """
        修改指定工具坐标系
